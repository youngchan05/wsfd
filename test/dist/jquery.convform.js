...
ConvState.prototype.printQuestion = function(){
    ...
    setTimeout(function(){
        ...
        this.scrollDown();
        var cloenItem = $('#convForm').clone(true);
        $('.test').append(cloenItem);
        $('.message.to.ready').append(cloenItem);
        if($('.message.to.ready').length === 3){
            $('.convFormDynamic').css('display','none');
            var userForm = $('<div class="test">'
            +'<div class="inpu-set"><label for="name"><span>*</span>이름</lable><input  id="name"type="text"></div>'
            +'<div class="inpu-set"><label for="email"><span>*</span>이메일</lable><input  id="email"type="text"></div>'
            +'<div class="inpu-set"><label for="num"><span>*</span>전화번호</lable><input  id="num"type="text"></div>'
            +'<div class="inpu-set check"><label for="agree">개인정보 보호정책에 동의합니다.</lable><input id="agree" type="checkbox"></div>'
            +'</div>')
            $('.message.to.ready').eq(2).append(userForm);

        }
        if($('.message.to.ready').length >= 4){
            $('.message .convFormDynamic').addClass('hide');
            $('.convFormDynamic').css('display','block');
        }
        ...
        for(var i in answers){
            if(answers.hasOwnProperty(i)){
                ...
                var option = $('<div class="option">'+answers[i].text+'</div>')
                    ...
                    $('.message.to.ready .options').on('click',function(){
                        return;
                    })
                this.wrapper.find('div.options').append(option);
                $(window).trigger('dragreset');
        ...
        */
        ...
        if(inputs.length) {
            ...
            switch(parameters.typeInputUi) {
                case 'input':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="convFormDynamic"><div align="right" class="options dragscroll"></div><input id="' + parameters.inputIdName + '" type="text" placeholder="'+ parameters.placeHolder +'" class="userInputDynamic"></><button type="submit" class="submit">'+parameters.buttonText+'</button><span class="clear"></span></form>');
                    break;
                case 'textarea':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="convFormDynamic"><div align="right" class="options dragscroll"></div></form>');
                    break;
                default :
